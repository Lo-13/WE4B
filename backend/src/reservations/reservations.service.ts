...
export class ReservationsService {
  ...
  async create(createReservationDto: CreateReservationDto): Promise<Reservation> {
    const room = await this.roomsService.findRoomEntity(Number(createReservationDto.roomId));
    const user = await this.usersRepository.findOne({ where: { id: Number(createReservationDto.userId) } });
    ...
    const selectedGameId = Number(createReservationDto.gameId ?? room.games?.[0]?.id);

    if (!selectedGameId) {
      throw new BadRequestException('No game is available for this room');
    }
    ...
    return this.toReservationDto(await this.findEntity(savedReservation.id));
  }
  ...
  async addComment(id: number, addCommentDto: AddCommentDto): Promise<Reservation> {
    ...
    return this.toReservationDto(await this.findEntity(id));
  }
